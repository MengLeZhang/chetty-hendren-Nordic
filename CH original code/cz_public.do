...
*	  kid_rank: kid family income rank at age specified by age_outcome #comment kid family = kid + spouse -- ? rank by cohort (deffo)
	  par_rank_n: parent family income rank #?average income between 1996 - 2000 (no conditioning on age/ birth cohort -- based on core sample, think its unconditional distribution)
	  par_bin: parent family income decile bin #
	  par_cz_orig: parent origin CZ # CZ before 1st
	  par_cz_dest: parent destination CZ #
	  e_o: expected rank in origin # separate script to run,
	  e_d: expected rank in destination #
	  d_e: difference in expected rank (d_e=e_d-e_o) #
      insample: dummy, = 1 if move distance > 100 miles and origin/destination # compute this from GIS; separate script
	    populations > 250k and = 0 otherwise # pop of CZ
	  ...
	  parmar_yrmovecz_pre: parent marital status dummy in year prior to move # need this raw data
...
use movers_v10_czwork_long.dta, clear *[Meng] WTF is this dataset?
...
* Fig IVa
# here is what we think this does
